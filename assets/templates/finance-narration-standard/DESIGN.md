...
- Book/product CTA: final hook line, bounded book card, 2-3 useful tags, and a soft action phrase only when narration supports it.
...
- Do not label the ending as `书名登场`, `背书推荐`, `工具书推荐`, `行动指令`, or `CTA`; write what the viewer should understand instead.
